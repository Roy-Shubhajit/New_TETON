...
Original file is located at
    https://colab.research.google.com/drive/1yZaaclJliAbR0SJTv16pbLwMJHAWf9_S
"""
...
#_force_cpu = str(os.environ.get("1", "")).strip().lower() in {
    #"1", "true", "yes", "on"
#}
_force_cpu = True
_n_jobs_env_raw = str(os.environ.get("PARALLEL_SINDY_N_JOBS", "")).strip()

if _force_cpu:
    GPU_AVAILABLE = False
    cp = np
    #print("⚠️ PARALLEL_SINDY_FORCE_CPU is set (using NumPy)")
else:
    try:
        import cupy as cp
        GPU_AVAILABLE = True
...
class SindyArgs:
    def __init__(self, win_len=5120, stride=None, overlap=0.5, d_max=2, simpl_rho=1.0, scale=1.0,
                 admm_rho=3.0, admm_overrelax=1.6, max_iters=250, fs=256.0,
                 win_sg=29, order=3, r_target_pc=0.9, k_min=700, k_max=1000000,
...
def _process_single_window(
    ...
    #print(f"Shape of Thetaz: {Thetaz.shape}")
    ...
    if np.any(active_mask):
        ...
        for _ in range(300):
            ...
            diff = float(xp.linalg.norm(Xi_deb - prev))
            if diff < 1e-5:
                break
            prev = Xi_deb.copy()
...
def process_data_in_windows(data, args, n_jobs=None):
    """
    Process dataset over overlapping/non-overlapping windows.
    Data format: [nodes, time] with shape (n, T).
    """
    n, T_raw = data.shape
    dt = 1.0 / args.fs
    ...
    starts = list(range(0, T_len - args.win_len + 1, stride))

    # GPU path: keep serial
    if GPU_AVAILABLE:
        results = []
        for window_index, w_start in tqdm(enumerate(starts), total=len(starts)):
            results.append(
                _process_single_window(
                    ...
                    Y_full=Y_full,
                    n=n,
                    idx_patterns=idx_patterns,
                    maps0=maps0,
                    edges0=edges0,
                    w_degree_base=w_degree_base,
                )
            )
        return results

    # CPU path: parallel over windows
    if n_jobs is None:
        if _n_jobs_env_raw:
            try:
                n_jobs = int(_n_jobs_env_raw)
            except ValueError:
                n_jobs = mp.cpu_count()
        else:
            n_jobs = mp.cpu_count()
    ...
    results = Parallel(
        ...
        for window_index, w_start in enumerate(starts)
    )

    results = sorted(results, key=lambda z: z["window_index"])
    return results